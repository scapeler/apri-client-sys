...
//			;
		...
		var keywords = colorDomain.map(function(name, ndx) {
  		...
  		y.domain([
    			d3.min(keywords, function(k) { return d3.min(k.values, function(v) { return v.val-2; } )}),
    			d3.max(keywords, function(k) { return d3.max(k.values, function(v) { return v.val+2; } )})
  			]);
