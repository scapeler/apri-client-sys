...
*/
	...
	var setSize = function(options) {
		containerSize = {};
		containerSize.margin		= {top: 25, right: 10, bottom: 50, left: 40};
		containerSize.width			= options.width?options.width:800;
		containerSize.graphicWidth	= containerSize.width -	containerSize.margin.left - containerSize.margin.right;
		containerSize.height		= options.height?options.height:500;
		containerSize.graphicHeight	= containerSize.height -	containerSize.margin.top - containerSize.margin.bottom;	
	}
	...
	this.createGraphic = function(inputData, options){
		...
		x = d3.time.scale().range([0, containerSize.graphicWidth]);
		y = d3.scale.linear().range([containerSize.graphicHeight, 0]);
...
//			;
		...
		if (options.groupBy) {
			var dataGroup = d3.nest()
    			...
    			.entries(_data);
			data = dataGroup.sort(function(a, b){
		...
		var keywordsUnSorted = colorDomain.map(function(name, ndx) {
		...
		var keywords = keywordsUnSorted;
//		 var keywords = keywordsUnSorted.sort(function(a, b){
...
//			});
		...
		var betweenTicksWidth = x(keywords[0].values[1].date) - x(keywords[0].values[0].date) ;
		...
		svg = d3.select(options.container)
			...
			.attr('class', 'trend')
			.append('g')
			.attr('transform', "translate("+containerSize.margin.left+","+containerSize.margin.top+")")
			;
...
//		var data = _data;
		...
		function adjustTextLabels(selection) {
			selection.selectAll('text')
				.attr('transform', 'translate(' + betweenTicksWidth / 2 + ',0)');
		}
/*
...
*/
		...
		svg.append('g')
			...
			.attr('transform', "translate(0, " + containerSize.graphicHeight + ")")
			.call(xAxis.tickSize(-containerSize.graphicHeight, 0, 0))
...
//			.attr('id', function(d) { return d.name.replace(/\s/g,'') } )
		...
		labels //= d3.select('#res-labels').
			...
			.attr('id', function(d) { return "s"+d.nameIdName+"l";})
...
//				var test = d3.select("#"+d.name.replace(/\s/g,'')+"")
				...
				trendLinesGroup.selectAll(".trendline")
					.style('stroke-width', '1px')
					.style('stroke-opacity', 0.2) //.25);
					;
				trendLinesGroup.selectAll("#s"+d.nameIdName+"s")
