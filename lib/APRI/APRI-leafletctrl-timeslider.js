...
var ApriLeafLetCtrlTimeSlider = ApriCore.ApriLeafLetCtrlTimeSlider = ApriCore.ApriLeafLetCtrlTimeSlider || Class.extend(function () { //ApriCore.ApriLeafLetCtrlBase(function () {
	...
	this.constructor = function(options) {
		...
		moduleInstanceId = APRI.UTIL.apriGuid();
		this.moduleInstanceId = moduleInstanceId;
		
		//alert(options.layer.options.properties.avgType);
		_options = options;
...
//		d3Slider.max(2100);
	...
	this.tickHandler = function(d) {
		if (document.documentMode) {
			...
			evt.detail = {layer:layer,sliderDate: d}
			evt.initEvent('tickEvent', true, false);
			//document.getElementById('target').dispatchEvent(evt); 
			document.dispatchEvent(evt);
		} else {
			//alert(layer.options.properties.avgType + ' ' + moduleInstanceId);
...
//		return formattedDate(date);
	...
	this.initControl = function(options) {
		...
		controlElement.setAttribute('moduleInstanceId', moduleInstanceId);
		controlElement.moduleInstanceId = moduleInstanceId;
